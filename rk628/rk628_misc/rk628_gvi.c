				       ...
				       struct rk628_gvi *gvi)
{
	...
	rk628_i2c_update_bits(rk628, GVI_COLOR_BAR_CTRL, COLOR_BAR_EN, 1);
}
...
{
	struct rk628_gvi *gvi = rk628_display_get_gvi_host();
	unsigned int rate;

	rk628_gvi_get_info(gvi);
	rate = rk628_gvi_get_lane_rate(gvi);
	...
	rk628_combtxphy_set_bus_width(rate);
	rk628_combtxphy_set_gvi_division_mode(gvi->division_mode);
	rk628_combtxphy_set_mode(rk628, PHY_MODE_VIDEO_GVI);
	rate = rk628_combtxphy_get_bus_width();
